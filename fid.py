...
def calculate_inception_stats(
    ...
    if _contains_npy_npz(image_path):
        dataset_obj = dataset.NPYDataset(path=image_path, resolution=64, slice_axis=0, max_size=num_expected, random_seed=seed)
        dist.print0("Using NPYDataset loader for .npy/.npz files")
    else:
        dataset_obj = dataset.ImageFolderDataset(path=image_path, max_size=num_expected, random_seed=seed)
        dist.print0("Using ImageFolderDataset loader for image files")
...
def main():
    ...
    python fid.py ref --data=datasets/processed_celeba-64x64 --dest=fid-refs/processed_celeba-64x64.npz
    """
